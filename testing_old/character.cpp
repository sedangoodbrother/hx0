...
void Character::setAction(const std::string& action) {
    ...
    if (actionTextures.find(action) != actionTextures.end()) {
        ...
        if (action == "parry_low" || action == "parry_high" || action == "parry_mid") {
            ...
            if (action == "dash_thrust") {
                // std::cout << "Dash Thrust activated!" << std::endl;
                // deactivateHitbox(); // Deactivate other hitboxes
                // actionStartTime = SDL_GetTicks(); // Record the start time of the action
                ...
                // // Set the dash velocity (2x normal velocity)
                // if (flip) {
                //     velocityX = -12; // Dash left
                // } else {
                //     velocityX = 12; // Dash right
                // }
    
                // // Set the dash duration
                // dashDuration = 1600; // 1.6 seconds
            }
...
void Character::updateState(Character& opponent) {
    ...
    // Handle dash_thrust logic
    if (currentAction == "dash_thrust") {
        // // Stop the dash if the duration exceeds 1.6 seconds
        // if (currentTime - actionStartTime > dashDuration) {
        //     resetToIdle();
        //     velocityX = 0; // Stop movement
        // }

        // // Stop the dash if the player's hitbox collides with the opponent's hurtbox/hitbox
        // if (SDL_HasIntersection(&hitbox, &opponent.hurtbox) || SDL_HasIntersection(&hitbox, &opponent.hitbox)) {
        //     resetToIdle();
        //     velocityX = 0; // Stop movement
        // }
    }
...
void Character::resetToIdle() {
    setAction("idle"); // Reset the player's state to idle
}
...
void Character::updatePosition(Character& opponent) {
    if (currentAction == "dash_thrust") return; // Skip position updates during the dash
...
std::vector<Command> player1Commands = {
    {{"up", "left", "attack"}, "parry_low", 1000}, // Up, Back, Attack within 1 second
    {{"down", "left", "attack"}, "parry_high", 1000}, // Down, Back, Attack within 1 second
    {{"left", "attack"}, "parry_mid", 1000}, // Back, Attack within 1 second
    {{"attack"}, "dash_thrust", 1600}, // Dash Thrust for Player 1
    {{"down", "attack"}, "strike_lowhigh", 1000}, // Strike Low-High for Player 1
    {{"up", "attack"}, "strike_highlow", 1000},   // Strike High-Low for Player 1
};

std::vector<Command> player2Commands = {
    {{"up", "right", "attack"}, "parry_low", 1000}, // Up, Forward, Attack within 1 second
    {{"down", "right", "attack"}, "parry_high", 1000}, // Down, Forward, Attack within 1 second
    {{"right", "attack"}, "parry_mid", 1000}, // Forward, Attack within 1 second
    {{"attack"}, "dash_thrust", 1600}, // Dash Thrust for Player 2
    {{"down", "attack"}, "strike_lowhigh", 1000},  // Strike Low-High for Player 2
    {{"up", "attack"}, "strike_highlow", 1000},    // Strike High-Low for Player 2
};

bool matchCommand(const InputHistory& history, const Command& command) {
    if (history.inputs.size() < command.sequence.size()) {
        std::cout << "Not enough inputs in history for command: " << command.action << std::endl;
        return false;
    }

    // Extract only the input strings from the input history
    std::vector<std::string> historyInputs;
    for (const auto& [input, timestamp] : history.inputs) {
        historyInputs.push_back(input);
    }

    // Check if all inputs in the command sequence are present in the history
    for (const auto& requiredInput : command.sequence) {
        auto it = std::find(historyInputs.begin(), historyInputs.end(), requiredInput);
        if (it == historyInputs.end()) {
            std::cout << "Input missing for command: " << command.action << ", required: " << requiredInput << std::endl;
            return false;
        } else {
            // Remove the matched input to prevent duplicate matches
            historyInputs.erase(it);
        }
    }

    std::cout << "Command matched: " << command.action << std::endl;
    return true;
}
        ...
        if (it != keyMappings.end()) {
            ...
            for (const auto& command : commands) {
                if (matchCommand(history, command)) {
                    std::cout << "Matched Command: " << command.action << std::endl;
                    character.setAction(command.action); // Trigger the command action
                    history.clear(); // Clear history after triggering a command
                    break;
                }
            }
        }
